...
namespace CutsceneEngineEditor
{
    ...
    public class TransformClipInspector : Editor
    {
        ...
        void DuringSceneGUI(SceneView sceneView)
        {
            ...
            {
                var clips = kv.Value
                    .Where(c => c?.asset is TransformClip)
                    .OrderBy(c => c.start)
                    .ThenBy(c => c.end)
                    .ToList();
        ...
        void DrawHandles(TransformClip clip)
        {
            if (!clip) return;
            var director = TimelineEditor.inspectedDirector;
            if (!director) return;
            ...
            if (destination)
            {
                worldPos = destination.position;
                worldRot = destination.rotation;
                scale = destination.localScale;
            }
            else
            {
                worldPos = clip.position;
                if (clip.applyPositionOffset) worldPos += initialPos;

                worldRot = Quaternion.Euler(clip.rotation);
                if (clip.applyRotationOffset) worldRot = Quaternion.Euler(clip.rotation + initialRot.eulerAngles);

                scale = clip.scale;
                if (clip.applyScaleOffset) scale += initialScale;
            }
            ...
            {
                case Tool.Move:
                    worldPos = Handles.PositionHandle(worldPos, Tools.pivotRotation == PivotRotation.Local ? worldRot : Quaternion.identity);
                    break;
                case Tool.Rotate:
                    worldRot = Handles.RotationHandle(worldRot, worldPos);
                    break;
                case Tool.Scale:
                    scale = Handles.ScaleHandle(scale, worldPos, worldRot, HandleUtility.GetHandleSize(worldPos));
                    break;
            }
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (destination)
                {
                    Undo.RecordObject(destination, "Edit Transform");
                    if (Tools.current == Tool.Move) destination.position = worldPos;
                    if (Tools.current == Tool.Rotate) destination.rotation = worldRot;
                    if (Tools.current == Tool.Scale) destination.localScale = scale;
                }
                else
                {
                    Undo.RecordObject(clip, "Edit Transform Clip");
                    if (Tools.current == Tool.Move)
                    {
                        clip.position = clip.applyPositionOffset ? worldPos - initialPos : worldPos;
                    }
                    if (Tools.current == Tool.Rotate)
                    {
                        Vector3 newEuler = worldRot.eulerAngles;
                        if (clip.applyRotationOffset) newEuler -= initialRot.eulerAngles;
                        clip.rotation = newEuler;
                    }
                    if (Tools.current == Tool.Scale)
                    {
                        clip.scale = clip.applyScaleOffset ? scale - initialScale : scale;
                    }
