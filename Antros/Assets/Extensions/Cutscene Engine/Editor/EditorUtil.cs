...
namespace CutsceneEngineEditor
{
    internal static class EditorUtil
    {
        ...
        /// <summary>
        /// Returns true if TimeineEditorWindow's preview mode is enabled.
        /// </summary>
        public static bool IsPreviewEnabled()
        {
            // Timeline 창이 닫혀 있으면 null
            ...
            var state =
                window.GetType()
                    .GetProperty("state", InstanceFlags)?
                    .GetValue(window);
            ...
            // 버전에 따라 property 또는 field일 가능성을 모두 처리
            if (stateType.GetProperty("previewMode", InstanceFlags)?
                    .GetValue(state) is bool propertyValue)
            {
                return propertyValue;
            }

            return stateType.GetField("previewMode", InstanceFlags)?
                       .GetValue(state) is bool fieldValue
                   && fieldValue;
        }
    }
