...
{
    public partial class FournaiseStatus : Status<FournaiseData, FournaiseComponent, StatusDurationController>, ITickOnTurnEnd
    {
       protected override FournaiseComponent CreateStatusComponent(FournaiseData data, in StatusContext context)
       {
          return new FournaiseComponent(data);
       }

       protected override StatusDurationController CreateStatusController(FournaiseData data, in StatusContext context)
       {
          return new StatusDurationController(data.Duration);
       }

       protected override void OnTick(FournaiseData data, in EntityStatusInfos statusInfos, in StatusContext context)
       {
	       base.OnTick(data, in statusInfos, in context);
	       if (!statusInfos.targetAddress.TryGetComponentRO<BattleCellComponent>(out _))
		       return;

	       var cellAspect = new BattleCellAspect(statusInfos.targetAddress);
