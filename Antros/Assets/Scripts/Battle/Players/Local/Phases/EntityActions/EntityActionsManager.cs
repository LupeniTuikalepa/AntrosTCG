...
namespace ATCG.Battle
{
	...
	{
		public static void GetActionsFor(this EntityAddress address, List<EntityAction> actions,
			LocalBattlePlayer player)
		{
			...
			if (address.Is<CapacityCasterAspect>(out var aspect))
			{
				...
				if (capacities.Count > GameMetrics.Current.MaxCapacityCountBeforeChooseModal)
				{
					actions.Add(new ChooseCapacityAction(player, aspect.GridMemberComponent.coordinates, capacities));
				}
				else
				{
					...
					{
						actions.Add(
							new CastCapacityAction(player, capacityData, aspect.GridMemberComponent.coordinates));
					}
				}
			}
		}
