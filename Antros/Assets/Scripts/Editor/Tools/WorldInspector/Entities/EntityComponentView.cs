...
namespace ATCG.Battle.Entities.EditorTools
{
    ...
    public sealed class EntityComponentView
    {
        ...
        public void Populate(VisualElement pane, World world, int entityId)
        {
            ...
            {
                InspectorLog.Warn($"GetMeta failed for entity {entityId}", e);
                pane.Add(Empty("(meta unavailable)"));
                return;
            }
            ...
            for (int id = 0; id < ComponentRegistry.MaxComponents; id++)
            {
                ...
                // The meta says this component is present, so a missing/failing store
                // here means the id->store mapping is inconsistent (e.g. registry drift
                // across plays). That's an anomaly worth surfacing, not swallowing.
                IComponentStore store;
                try { store = world.GetStore(id); }
                catch (Exception e) { InspectorLog.Warn($"GetStore({id}) for {type.Name} threw", e); continue; }
                if (store == null)
                {
                    InspectorLog.Warn($"No store for component id {id} ({type.Name}) although the entity's mask has it — possible id mapping drift.");
                    continue;
                }

                object boxed;
                try { boxed = store.GetBoxed(entityId); }
                catch (Exception e) { InspectorLog.Warn($"GetBoxed({entityId}) on {type.Name} store threw", e); continue; }
                if (boxed == null)
                    continue;
