...
namespace GameSourceGeneration
{
    ...
    public class EntityAspectImplementationGenerator : IIncrementalGenerator
    {
        ...
        private static string GenerateAspectImplementation(AspectInfo info)
        {
            ...
            sb.AppendLine($"    {{");
            string infoStructNameVariable = FirstLetterLowerCase(info.structName);

            sb.AppendLine($"        public static bool Is{info.structName}(this EntityAddress address, out {info.structName} {infoStructNameVariable})");
            sb.AppendLine($"        {{");
            sb.AppendLine($"            if(address.Is{info.structName}())");
            sb.AppendLine($"            {{");
            sb.AppendLine($"                {infoStructNameVariable} = new {info.structName}();");
            sb.AppendLine($"                return true;");
            sb.AppendLine($"            }}");
            sb.AppendLine($"            {infoStructNameVariable} = default;");
            ...
            if (info.factoryInfosTypes != null)
            {
                ...
                sb.AppendLine($"        {{");
                sb.AppendLine($"            public readonly EntityAddress EntityAddress;");
                // Une propriété factory par composant
                foreach (var type in info.componentTypes)
                {
                    sb.AppendLine($"            public {type} {type}");
                    sb.AppendLine($"            {{");
                    sb.AppendLine($"                get => EntityAddress.GetComponent<{type}>();");
                    sb.AppendLine($"                set => EntityAddress.AddOrSetComponent<{type}>(value);");
                    sb.AppendLine($"            }}");
                ...
                sb.AppendLine($"            var factory = new {ComponentsFactoryClassName}();");
