...
public class CapacityDataGenerator : IIncrementalGenerator
{
    ...
    private static void Emit(SourceProductionContext ctx, CapacityDataPartialFileInfos info)
    {
        ...
        foreach ((string name, string identifier) in ordered)
            sb.Append("    public const string ").Append(identifier)
              .Append(" = \"").Append(Escape(name)).AppendLine("\";");

        sb.AppendLine();

        // ordered declared-steps array (drives editor reconciliation)
        sb.Append("    public static readonly string[] DeclaredSteps = { ");
        sb.Append(string.Join(", ", ordered.Select(o => identifierByName[o.name] is var id ? id : o.name)));
        sb.AppendLine(" };");
        ...
        foreach ((string name, string identifier) in ordered)
            sb.Append("    public global::ATCG.Capacities.CapacityStepData ")
